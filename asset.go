...
func redPacket(cookie string, rpc chan []RedList) {
	...
	a := AutoGenerated{}
	req := httplib.Get(`https://m.jingxi.com/user/info/QueryUserRedEnvelopesV2?type=1&orgFlag=JD_PinGou_New&page=1&cashRedType=1&redBalanceFlag=1&channel=3&_=` + fmt.Sprint(time.Now().Unix()) + `&sceneval=2&g_login_type=1&callback=jsonpCBKA&g_ty=ls`)
	req.Header("User-Agent", ua())
